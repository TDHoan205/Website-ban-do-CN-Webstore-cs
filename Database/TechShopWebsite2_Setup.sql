...
BEGIN
    CREATE TABLE Cart_Items (
        ...
        variant_sentinel AS (ISNULL(variant_id, -999999)) PERSISTED,
        FOREIGN KEY (product_id) REFERENCES Products(product_id),
        FOREIGN KEY (variant_id) REFERENCES ProductVariants(variant_id)
    );
...
BEGIN
    ...
    IF COL_LENGTH('dbo.Cart_Items', 'variant_sentinel') IS NULL
        ALTER TABLE Cart_Items ADD variant_sentinel AS (ISNULL(variant_id, -999999)) PERSISTED;
...
BEGIN
    ...
    VALUES (N'Samsung Việt Nam', N'Nguyễn Văn A', '0912345678', 'contact@samsung.vn', N'TP. Hồ Chí Minh'),
           ...
           (N'ASUS Việt Nam', N'Phạm Thị D', '0933112233', 'contact@asus.vn', N'TP. Hồ Chí Minh'),
           (N'Sony Việt Nam', N'Đỗ Văn E', '0944556677', 'contact@sony.vn', N'Hà Nội');
...
BEGIN
    ...
    VALUES (N'admin', 'admin123', 'admin@techshop.vn', N'Quản trị viên', '0917111111', N'TP. Hồ Chí Minh', 'Admin', 1),
           ...
           (N'customer3', 'customer123', 'customer3@email.com', N'Khách hàng 3', '0919444444', N'Đà Nẵng', 'Customer', 1),
           (N'customer4', 'customer123', 'customer4@email.com', N'Khách hàng 4', '0919555555', N'Cần Thơ', 'Customer', 1);
...
BEGIN
    ...
    (N'iPad Pro M4 11-inch', N'Máy tính bảng cao Aấp với màn hình OLED và chip M4.', 28990000, 29990000, 40, 4.8, 1, 1, 3, '{"cpu": "Apple M4", "screen": "11 inch"}', '/images/products/ipadprom4.png', 3, 2, 1),
    (N'Sony WH-1000XM5', N'Tai nghe chống ồn không dây hàng đầu.', 7990000, 8490000, 60, 4.7, 0, 1, 5, '{"type": "Over-ear", "battery": "30h"}', '/images/products/sonywh1000xm5.png', 4, 5, 1);
...
BEGIN
    ...
    (4, N'Space Black', '256GB', '8GB', N'Space Black / 256GB', 28990000, 29990000, 20, 1),
    (4, N'Silver', '256GB', '8GB', N'Silver / 256GB', 28990000, 29990000, 20, 2),
    (5, N'Đen', NULL, NULL, N'Màu Đen', 7990000, 8490000, 30, 1),
    (5, N'Bạc', NULL, NULL, N'Màu Bạc', 7990000, 8490000, 30, 2);
    PRINT 'Inserted ProductVariants data';
END
GO

-- Product Images
IF NOT EXISTS (SELECT * FROM ProductImages)
BEGIN
    INSERT INTO ProductImages (product_id, variant_id, image_url, is_primary, is_thumbnail, display_order, alt_text)
    VALUES
    (1, NULL, '/images/products/iphone15promax.png', 1, 1, 1, N'iPhone 15 Pro Max'),
    (2, NULL, '/images/products/galaxys24ultra.png', 1, 1, 1, N'Samsung Galaxy S24 Ultra'),
    (3, NULL, '/images/products/macbookairm3.png', 1, 1, 1, N'MacBook Air M3'),
    (4, NULL, '/images/products/ipadprom4.png', 1, 1, 1, N'iPad Pro M4'),
    (5, NULL, '/images/products/sonywh1000xm5.png', 1, 1, 1, N'Sony WH-1000XM5');
    PRINT 'Inserted ProductImages data';
END
GO
...
BEGIN
    INSERT INTO Inventory (product_id, quantity_in_stock, last_updated_date)
    VALUES
    (1, 100, GETDATE()),
    (2, 80, GETDATE()),
    (3, 50, GETDATE()),
...
BEGIN
    INSERT INTO FAQs (question, answer, category, keywords, priority, is_active)
    VALUES
    (N'Làm sao để đặt hàng?', N'Bạn có thể thêm sản phẩm vào giỏ và tiến hành thanh toán trực tuyến.', 'order', 'đặt hàng, mua hàng', 1, 1),
    (N'Cửa hàng có miễn phí vận chuyển không?', N'Miễn phí vận chuyển cho đơn hàng từ 500.000đ.', 'shipping', 'vận chuyển, ship, phí', 1, 1),
    (N'Tôi có thể đổi trả hàng không?', N'Có, bạn có thể đổi trả trong vòng 7 ngày nếu lỗi do nhà sản xuất.', 'return', 'đổi trả, bảo hành', 1, 1),
    (N'Phương thức thanh toán nào được chấp nhận?', N'Chúng tôi chấp nhận thẻ tín dụng, chuyển khoản và thanh toán khi nhận hàng (COD).', 'payment', 'thanh toán, cod, thẻ', 1, 1),
    (N'Hàng có chính hãng không?', N'100% sản phẩm tại TechStore là hàng chính hãng được bảo hành toàn quốc.', 'product', 'chính hãng, giả', 1, 1);
    PRINT 'Inserted FAQs data';
END
GO
